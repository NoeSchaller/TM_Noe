class Demonstration extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(520,10, 'Lwheel', {color: '#000', backgroundColor: '#999', padding: 3})
            .setInteractive().on('pointerdown', () => {
                if(this.simulation.rb.state.wheel.left.on){
                    this.simulation.rb.state.wheel.left.on = false
                }else{
                    this.simulation.rb.state.wheel.left.on = true  
                }
            })

        this.add.text(520,35, '+', {color: '#000', backgroundColor: '#999', padding: 3})
            .setInteractive().on('pointerdown', () => {
                this.simulation.rb.state.wheel.left.speed += 1
            })
        ...
        this.add.text(560,35, '-', {color: '#000', backgroundColor: '#999', padding: 3})
            ...
            .setInteractive().on('pointerdown', () => {
                if(this.simulation.rb.state.wheel.right.on){
                    this.simulation.rb.state.wheel.right.on = false
                }else{
                    this.simulation.rb.state.wheel.right.on = true  
                }
            })

        this.add.text(520,85, '+', {color: '#000', backgroundColor: '#999', padding: 3})
            .setInteractive().on('pointerdown', () => {
                this.simulation.rb.state.wheel.right.speed += 1
            })
        ...
        this.add.text(520,115, 'LLed', {color: '#000', backgroundColor: '#999', padding: 3})
            .setInteractive().on('pointerdown', () => {
                if(this.simulation.rb.state.led.left){
                    this.simulation.rb.state.led.left = false
                }else{
                    this.simulation.rb.state.led.left = true  
                }
            })

        this.add.text(520,145, 'RLed', {color: '#000', backgroundColor: '#999', padding: 3})
            .setInteractive().on('pointerdown', () => {
                if(this.simulation.rb.state.led.right){
                    this.simulation.rb.state.led.right = false
                }else{
                    this.simulation.rb.state.led.right = true  
                }
            })
    ...
    update() {
        if(this.ray){
            console.log(this.simulation.rb.getDistance())
        }
    }
}
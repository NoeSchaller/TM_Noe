class I2cPlus {
    ...
    protected robot: MaqueenPlus;
    protected colors: number[];
    protected buffer: number[];

    constructor(robot: MaqueenPlus) {
      this.robot = robot;
      this.colors = [
    ...
    write(adresse: number, byte: Uint16Array) {
      if (adresse == 0x10) {
        ...
        if (register == 0x00) {
          ...
          this.buffer.push(
            this.robot.motorRight.power,
            this.robot.motorRight.direction,
            this.robot.motorLeft.power,
            this.robot.motorLeft.direction
          );
          ...
          this.buffer.push(
            this.robot.motorRight.angle % 256,
            (this.robot.motorRight.angle >> 8) % 256,
            this.robot.motorLeft.angle % 256,
            (this.robot.motorLeft.angle >> 8) % 256
          );
        }
    ...
    read(adresse: number, number: number) {
      if (adresse == 0x10) {
        let get = [];

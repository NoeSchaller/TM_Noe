...
class Infrared {
    constructor(scene, reference, x, y, radius = 2, stateBlack = true) {
        ...
        this.captor = scene.matter.add
            .gameObject(scene.add.circle(reference.x + x, reference.y + y, radius, 0xffffff), scene.matter.add.circle(reference.x + x, reference.y + y, 1))
            .setCollidesWith(0)
            .setDepth(2);
    }
    isMarked() {
        for (let i = 0; i < this.scene.marks.length; i++) {
            if (this.scene.matter.overlap(this.captor, this.scene.marks[i].body)) {
                ...
                const xAngle0 = Math.cos(-(mark.angle / 180) * Math.PI) *
                    (this.captor.x - mark.position.x) -
                    Math.sin(-(mark.angle / 180) * Math.PI) *
                        (this.captor.y - mark.position.y), yAngle0 = Math.sin(-(mark.angle / 180) * Math.PI) *
                    (this.captor.x - mark.position.x) +
                    Math.cos(-(mark.angle / 180) * Math.PI) *
                        (this.captor.y - mark.position.y);
                const color = this.scene.textures.getPixel(xAngle0 / mark.scale.x + mark.body.width / 2, yAngle0 / mark.scale.y + mark.body.height / 2, mark.picture);
                if (color !== null) {
    ...
    update() {
        this.captor.setPosition(this.reference.x +
            this.deltaOrigin *

...
class MaqueenLite {
    constructor(scene, name, x, y, angle) {
        ...
        this.leftMotor = new Motor(scene, this.body, -35, 18, 9, 43, { x: -10, y: -4 }, { x: -10, y: 40 }, speedGrowth);
        this.rightMotor = new Motor(scene, this.body, 35, 18, 9, 43, { x: 10, y: -4 }, { x: 10, y: 40 }, speedGrowth);
    ...
    setPosition(x, y) {
        ...
        this.leftMotor.wheel.setPosition(x +
            this.leftMotor.deltaOrigin *
                Math.cos(this.leftMotor.rotationOrigin + this.body.rotation), y +
            this.leftMotor.deltaOrigin *
                Math.sin(this.leftMotor.rotationOrigin + this.body.rotation));
        this.rightMotor.wheel.setPosition(x +
            this.rightMotor.deltaOrigin *
                Math.cos(this.rightMotor.rotationOrigin + this.body.rotation), y +
            this.rightMotor.deltaOrigin *
                Math.sin(this.rightMotor.rotationOrigin + this.body.rotation));
    }
    setAngle(deg) {
        this.body.setAngle(deg);
        this.leftMotor.wheel.setPosition(this.body.x +
            this.leftMotor.deltaOrigin *
                Math.cos((deg / 180) * Math.PI + this.leftMotor.rotationOrigin), this.body.y +
            this.leftMotor.deltaOrigin *
                Math.sin((deg / 180) * Math.PI + this.leftMotor.rotationOrigin));
        this.leftMotor.wheel.setAngle(deg);
        this.rightMotor.wheel.setPosition(this.body.x +
            this.rightMotor.deltaOrigin *
                Math.cos((deg / 180) * Math.PI + this.rightMotor.rotationOrigin), this.body.y +
            this.rightMotor.deltaOrigin *
